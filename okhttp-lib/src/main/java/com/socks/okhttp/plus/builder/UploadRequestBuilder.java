...
public class UploadRequestBuilder extends RequestBuilder {
    ...
    public UploadRequestBuilder file(Pair<String, File> file) {
        if(files == null) {
            files = new ArrayList<Pair<String, File>>();
        }
        files.add(file);
        return this;
    }
    ...
    private static void addFiles(MultipartBody.Builder builder, List<Pair<String, File>> files) {
        if (files != null) {
            for (int i = 0; i < files.size(); i++) {
                Pair<String, File> filePair = files.get(i);
                addFile(builder, filePair);
            }
        } else {
    ...
    private static void addFile(MultipartBody.Builder builder, Pair<String, File> filePair) {
